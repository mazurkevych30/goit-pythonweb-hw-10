...
async def update_contact(
    ...
    contact = await contact_secvice.update_contact(contact_id, body)
    if contact is None:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail=f"Contact with id {contact_id} not found",
...
async def delete_contact(contact_id: int, db: AsyncSession = Depends(get_db)):
    contact_secvice = ContactsService(db)
    await contact_secvice.remove_contact(contact_id)
    return None
...
async def search_contacts(
    ...
    contact_secvice = ContactsService(db)
    return await contact_secvice.search_contacts(query, limit, offset)


@router.get("/upcoming_birthdays/", response_model=list[BaseContact])
async def get_upcoming_birthdays(
    days_ahead: int = Query(7, ge=1, le=31), db: AsyncSession = Depends(get_db)
):
    contact_secvice = ContactsService(db)
    return await contact_secvice.get_upcoming_birthdays(days_ahead)
